...
@Client.on_message(    filters.user(SUDO_USERS) & filters.command(["اسكت", "وقف"], prefixes=f"{HNDLR}"))
@authorized_users_only
...
@Client.on_message(    filters.user(SUDO_USERS) & filters.command(["كمل"], prefixes=f"{HNDLR}"))
@authorized_users_only
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(                f"**⏸ تم إيقاف التشغيل مؤقتًا.**\n\n• يمكنك ارجاع التشغيل بواسطه ارسال امر  » `{HNDLR}اسكت_كمل`"            )
        except Exception as e:
            await m.reply(f"**هناك خطأ ** \n`{e}`")
    else:
        await m.reply("** ❌ لايوجد صوت مشتغله !**") 
@Client.on_message(    filters.user(SUDO_USERS) & filters.command(["اسكت_كمل"], prefixes=f"{HNDLR}"))
@authorized_users_only
